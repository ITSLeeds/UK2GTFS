...
importMSN <- function(file, silent = TRUE){
  ...
  # Physical Station
  if(!silent){message(paste0(Sys.time()," importing Physical Station"))}
  station = raw[types == "A"]
  station = station[seq(from = 2, to = length(station))]


  station = iotools::dstrfw(x = station,
                            col_types = rep('character',17-1),
                            widths = c(1,4,26+4,1,7,3,3,3,5,1,5,2,1,1,11,3))

  names(station) = c("Record Type","Reserved1","Station Name",
  ...
  if(!silent){message(paste0(Sys.time()," importing GB Timetable numbers"))}
  timetable = raw[types == "B"]
  #timetable = timetable[seq(from = 2, to = length(station))]


  timetable = iotools::dstrfw(x = timetable,
  ...
  # Alias
  if(!silent){message(paste0(Sys.time()," importing Alias"))}
  alias = raw[types == "L"]
  alias = iotools::dstrfw(x = alias,
...
strip_whitespace = function(df){
  for(i in seq(from = 1, to = ncol(df))){
    val = trimws(df[[i]], which = "right")
    val = ifelse(grepl("^\\s*$", val),NA,val)
    df[[i]] = val
  }
  return(df)
}
